...
var (
	runCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if eventPostProcessor != "" {
				jsEpp, err := utils.NewJavascriptPostProcessor(eventPostProcessor)
				if err != nil {
					fmt.Fprintln(os.Stderr, "Error loading the event post processor:", err)
					return err
				}
				compute.eventPostProcessor = jsEpp
			}
	...
	exportSchemaCmd = &cobra.Command{
		...
		Short: "Export a json schema for a compute manifest or plugin manifest.",
		Long:  "Exports a json schema.  Takes either 'compute' or 'plugin' as arguments and exports the respective JSON schema.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "plugin":
				...
				schema := jsonschema.Reflect(&plugin)
				json, err := json.MarshalIndent(schema, "", "  ")
				if err != nil {
					return fmt.Errorf("error exporting compute schema: %s", err)
				}
				fmt.Println(string(json))
			case "compute":
				computeManifest := cloudcompute.ComputeManifest{}
				schema := jsonschema.Reflect(&computeManifest)
				json, err := json.MarshalIndent(schema, "", "  ")
				if err != nil {
					return fmt.Errorf("error exporting compute schema: %s", err)
				}
...
func initializeCommands() {
	...
	rootCmd.PersistentFlags().StringVarP(&computeFile, "computeFile", "c", "", "Path to compute file")
	...
	runCmd.Flags().StringVarP(&eventPostProcessor, "eventPostProcessor", "p", "", "Optional path to a javascript event post processor for modifying event data (compute manifest, etc) after it is generated by the event generator")
